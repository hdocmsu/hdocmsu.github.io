---
layout: post
title: "<span style='color: var(--global-post-pre-title-color);'>AoMP-001-Q:</span> <span style='color: var(--global-post-title-color);'>Classic Artifacts in Radial MRI and a Mysterious Artifact (re-discovered)</span>"
date: 2026-02-08 10:10:10
description: A deep dive into understanding the physics behind classic radial MRI artifacts and a mysterious artifact that was recently re-discovered. And how to mitigate them.
tags: mri math physics # lowercase only
categories: challenges # lowercase only
img: /assets/blog/aomp-001/aomp-001-slide-001.png
redirect: #https://hdocmsu.github.io/projects/canon_1_dlr/
toc: 
  beginning: true
citation: true
disqus_comments: true
selected_post: true
---

Do you know that, the first MRI image, acquired in 1973 by Paul Lauterbur, was actually a radial MRI image? 

Although used to create the first ever MRI image, the radial MRI acquisition scheme is not commonly used in clinical practice today. Instead, the Cartesian acquisition scheme is more widely adopted in clinical MRI due to its simplicity and efficiency. However, radial MRI has unique advantages such as reduced motion artifacts and applicability in certain applications, which has led to its continued use in research and specialized clinical settings. 

Radial MRI has been used in liver MRI to reduce motion artifacts and capture the dynamic contrast enhancement of liver lesions. It also has been used in musculoskeletal MRI to generate CT-like contrast via ultrashort echo time (UTE) and zero echo time (ZTE) imaging. 

Although not strictly radial, the PROPELLER/BLADE/JET MRI acquisition scheme, which samples k-space in a rotating fashion, has been widely used in brain and body MRI to reduce motion artifacts, where the artifacts are similar to those in radial MRI.

In this post, we will challenge you to identify the causes of the classic artifacts in radial MRI and propose solutions to mitigate them. During the process, we will also re-discover a mysterious artifact and strategy to mitigate it.

# The "Halo" Artifact

<figure style="margin: 0rem; padding-top: 1.5rem;">
  <img src="/assets/blog/aomp-001/halo1.png" alt="Halo Artifact in Radial MRI" style="max-width: 100%; height: auto;">
</figure>

<figure style="margin: 0rem; padding-top: 2.5rem; padding-bottom: 2.0rem;">
  <img src="/assets/blog/aomp-001/halo2.png" alt="Halo Artifact in Radial MRI - Question" style="max-width: 100%; height: auto;">
</figure>

# The "Freaking" Artifact

<figure style="margin: 0rem; padding-top: 1.5rem;">
  <img src="/assets/blog/aomp-001/streaking1.png" alt="Streaking Artifact in Radial MRI" style="max-width: 100%; height: auto;">
</figure>

<figure style="margin: 0rem; padding-top: 2.5rem; padding-bottom: 2.0rem;">
  <img src="/assets/blog/aomp-001/streaking2.png" alt="Streaking Artifact in Radial MRI - Question" style="max-width: 100%; height: auto;">
</figure>


# How to Mitigate the "Freaking Streaking" Artifact?

<figure style="margin: 0rem; padding-top: 1.5rem; padding-bottom: 2.0rem;">
  <img src="/assets/blog/aomp-001/streaking3_q.png" alt="Streaking Artifact in Radial MRI - Question 2" style="max-width: 100%; height: auto;">
</figure>


# The "Mysterious" Artifact

<div class="video-container" style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; max-width: 100%; margin: 2rem 0 0;">
  <iframe 
    style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;"
    src="https://www.youtube.com/embed/ND1XyVJPcBA" 
    frameborder="0" 
    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" 
    allowfullscreen>
  </iframe>
</div>
<div style="font-size: 0.85em; font-style: italic; text-align: center; margin-bottom: 0rem;">Reconstructed images from every iteration using the Original Conjugate Gradient SENSE Algorithm (see <a href="https://youtu.be/ND1XyVJPcBA" target="_blank">YouTube</a> if the video does not load)</div>

<figure style="margin: 0rem; padding-top: 2.5rem; padding-bottom: 2.0rem;">
  <img src="/assets/blog/aomp-001/mystery1.png" alt="Mysterious Artifact in Radial MRI" style="max-width: 100%; height: auto;">
</figure>


# How to Mitigate the "Mysterious" Artifact?

<figure style="margin: 0rem; padding-top: 1.5rem; padding-bottom: 2.0rem;">
  <img src="/assets/blog/aomp-001/mystery2_q.png" alt="Mysterious Artifact in Radial MRI - Question" style="max-width: 100%; height: auto;">
</figure>
